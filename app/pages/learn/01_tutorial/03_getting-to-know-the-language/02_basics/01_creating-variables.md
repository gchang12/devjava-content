...
- **Local Variables** Similar to how an object stores its state in fields, a method will often store its temporary state in local variables. The syntax for declaring a local variable is similar to declaring a field (for example, `int count = 0;`). There is no special keyword designating a variable as local; the locality of a variable is determined by the location in which the variable is declared — which is between the opening and closing braces of a method. As such, local variables are only visible to the methods in which they are declared; they are not accessible from the rest of the class.
...
Every programming language has its own set of rules and conventions for the kinds of names that you're allowed to use. The Java programming language is no different. The rules and conventions for naming your variables can be summarized as follows:

- Variable names are case-sensitive. A variable's name can consist of an unlimited-length sequence of Unicode letters and digits, beginning with a letter, the dollar sign `$`, or the underscore character `_`. The convention, however, is to always begin your variable names with a letter, not a `$` or a `_`. Additionally, the dollar-sign character, by convention, is never used at all. You may find some situations where auto-generated names will contain the dollar sign, but you should always avoid using it in your own variable names. A similar convention exists for the underscore character; while it's technically legal to begin your variable's name with `_`, this practice is discouraged. Whitespace is not permitted.
...
- If the name you choose consists of only one word, spell that word in all lowercase letters. If it consists of more than one word, capitalize the first letter of each subsequent word. The names `gearRatio` and `currentGear` are prime examples of this convention. For a variable that store a constant value, such as `static final int NUM_GEARS = 6`, the convention differs slightly: Capitalize every letter and separate subsequent words with the underscore character. Convention dictates that the underscore character must never be used elsewhere.
